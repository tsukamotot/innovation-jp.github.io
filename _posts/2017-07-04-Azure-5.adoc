...
## 検索ワード指定でぐるなび検索
苦肉の策で「◯◯が食べたい。」というメッセージの場合は、◯◯（料理名を想定）を LINE ユーザ ID をキーに Redis Cache　に保存し、位置情報せびるようにします。
...
位置情報を受信した際に、Redis Cache に登録された料理名取得し、次回の食べたいメッセージに備えて削除し、ぐるなびの API に取得した料理名を投げ、検索結果を返信します。
```
// 位置情報受信処理
case('location'):
    async.waterfall([
...
break;
```
何も入力がない場合は、強制的に検索ワードはパンケーキとなります。


## お話できるようにする
Repl-AI 用のユーザIDを払い出し、そのIDを利用する事で同一の人物として、お話できるという仕組みになっています、ですのでキーとして LINE ユーザ ID を利用し Repl-AI のユーザ　ID　を管理します。

その後、払い出されたユーザIDとメッセージを API に投げる事で、レスポンスにお返事メッセージが入っていますので、お返事メッセージを LINE に プッシュします。
```
async.waterfall([
	// Repl-AI のユーザID登録確認
	function(callback) {
...
画像解析も念のため、弊社のフリー素材 KTN さんを利用します。

お。相変わらず。

## 感想
Function App ですが個人的には UI が lambda よりスマートな感じがしました、Node.js のモジュールインストールは、コンソールでの操作が出来るので npm install で普通に出来ます、サーバレスとは。

async を利用し作り直した部分以外（関数等）は、コピペでいけるかと思いきや、console.log が利用できずエラーとなるという罠がありました。

また、Function App は1度起動するまでが遅いという欠点がありました、あくまでも batch 用関数なのかなと思いました。

総括すると、サーバレスとか関数とか言語にとらわれずに、なんでもやってみると楽しいもんだな〜と思いました、あれ？作文？？
