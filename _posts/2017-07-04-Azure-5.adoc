...
## 感想
Function App ですが個人的には UI が lambda よりスマートな感じがしました、Node.js のモジュールインストールは、なんとコンソールで操作が出来るので npm install で普通に出来ます、サーバレスとは…。

async を利用し作り直した部分以外（関数等）は、コピペでいけるかと思いきや、console.log が利用できずエラーとなる罠がありました、context.log じゃないとダメ！

Repl-AI のシナリオも今後作ってみたいですね。

統括すると、サーバレス関数とか、クラウドサービスとか、使用言語にとらわれずに、なんでもやってみると楽しいもんだな〜と思いました。あれ？作文？？
