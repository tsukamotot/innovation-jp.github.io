...
という事でまずはサンプルデータを使用しろという事なので早速ログイン、新しい実験を作成ます、MS ぽくない UI な気がしますが、左下の 「+ NEW」をクリック。
...
学習アルゴリズムと分析モデルを構築します、線形回帰（Linear Regression）モジュールを、トレーニングは（Train Model）モジュールを利用して行うという事で
それぞれ検索しキャンバスにセットし、分配した75%のデータと線形回帰の出力ポートをトレーニングに接続。
